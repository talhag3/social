<?php
    class Follow extends User {
        ...
        public function checkFollow($followID, $user_id){
            ...
            $stmt->bindParam(":followerID", $followID, PDO::PARAM_INT);
            $stmt->execute();
            ...
            if($this->loggedIn() === true){
                if($profileID != $user_id){
                    if($data['receiver'] == $profileID){
                        //followig button 
                        return "<button class='f-btn follow-btn' data-follow='".$profileID."' data-profile='".$followID."'>Following</button>";
                    }else{
                        //follow button
                        return "<button class='f-btn following-btn follow-btn' data-follow='".$profileID."' data-profile='".$followID."'><i class='fa fa-user-plus'></i>Follow</button>";
                    }
                }else{
                    //edit button
                    return "<button class='f-btn' onclick=location.href='profileEdit.php'>Edit Profile</i></button>";

                }
            }else{
                return "<button class='f-btn' onclick=location.href='index.php'><i class='fa fa-user-plus'>Follow</i></button>";
            }
        }
        ...
        public function follow($followID, $user_id, $profileID){
            ...
            $stmt = $this->pdo->prepare("SELECT `user_id`, `following`, `followes` FROM `users` LEFT JOIN `follow` ON `sender` = :user_id AND CASE WHEN `receiver` = :user_id THEN `sender` = `user_id` END WHERE `user_id` = :profileID");
