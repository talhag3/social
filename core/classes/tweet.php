...
use JetBrains\PhpStorm\Internal\ReturnTypeContract;
    ...
    class Tweet extends User {
                ...
                echo '<div class="all-tweet">
                            ...
                            <div class="t-show-inner">
                             ...
                             '.((!empty($tweet->retweetMsg) && $tweet->tweetID === $retweet['tweetID'] or $tweet->retweetID > 0) ? '
                                <div class="t-show-popup" data-type="'.$tweet->tweetID.'">
        ...
        public function getUserTweets($user_id){
            $stmt = $this->pdo->prepare("SELECT * FROM `tweets` LEFT JOIN `users` ON `tweetBy` = `user_id` WHERE `tweetBy` = :user_id AND `ertweetID` = '0' OR `retweetBy` = :user_id");
            $stmt->bindParam(":user_id", $user_id, PDO::PARAM_INT);
            $stmt->execute();
        ...
        public function retweet($tweet_id, $user_id, $get_id, $comment){
            $stmt = $this->pdo->prpare("UPDATE `tweets` SET `retweetCount` = `retweetCount` +1 WHERE `tweetID` = :tweet_id");
            $stmt->bindParam(":tweet_id", $tweet_id, PDO::PARAM_INT);
            $stmt->execute();
        ...
        public function countTweets($user_id){
            $stmt = $this->pdo->prepare("SELECT COUNT(`tweetID`) AS `totalTweets` FROM `tweeets` WHERE `tweetBy` = :user_id AND `retweetID` = '0' OR `retweetBy` = :user_id");
            $stmt->bindParam(":user_id", $user_id, PDO::PARAM_INT);
            $stmt->execute();
        ...
        public function getTweetsByHash($hashtag){
            ...
            $stmt->bindValue(":hashtag", '%#'.$hashtag.'%', PDO::PARAM_INT);
            $stmt->execute();
            return $stmt->fetchAll(PDO::FETCH_OBJ);
        }
