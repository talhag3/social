$(function(){
    $(document).on('click', '#messagePopup', function(){
        var getMessages = 1;
        $.post('http://localhost/twitter/core/ajax/messages.php', {ShowMessages:getMessages}, function(data){
    ...
    $(document).on('click', '.people-message', function(){
        ...
        $(document).on('click', '.deleteMsg', function(){
            ...
            $(document).on('click', '.delete', function(){
                $.post('http://localhost/twitter/core/ajax/messages.php', {deleteMsg:messageID}, function(data){
                    $('.message-del-inner').height('0px');
                    getMessages();
                });
            });
